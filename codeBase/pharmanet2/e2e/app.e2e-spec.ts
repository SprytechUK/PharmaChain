...
describe('Starting tests for pharmanet1', function() {
  ...
  it('website title should be pharmanet1', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('pharmanet1');
    })
  });

  it('network-name should be pharmanet@0.2.8-deploy.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('pharmanet@0.2.8-deploy.1.bna');
    });
  });

  it('navbar-brand should be pharmanet1',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('pharmanet1');
    });
  });
    ...
    it('Admin table should have 5 columns',() => {
      page.navigateTo('/Admin');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(5); // Addition of 1 for 'Action' column
      });
